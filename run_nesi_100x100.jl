...
for j in 1:n_ident
    v = A_T_final[:, j]
    if round(Int, v[K1_idx]) == 1 && round(Int, v[β1_idx]) == -1
...
for j in 1:n_nonident
    v = N_clean[:, j]
    if round(Int, v[β1_idx]) == round(Int, v[K1_idx]) && abs(round(Int, v[β1_idx])) >= 1
        other_sum = sum(abs.(round.(Int, v[[i for i in 1:n_params if i != β1_idx && i != K1_idx]])))
        if other_sum == 0
            gene1_nonident_idx = n_ident + j
            break
        end
    end
end
...
n_extra_guesses = 7  # More restarts for smoother results
...
ψ_vals, ll_vals = ReparamTools.profile_target(
    ...
    method=:LN_BOBYQA, optmaxtime=90.0  # More time per point
)
...
serialize("iir_100x100_results.jls", results)
println("\nResults saved to iir_100x100_results.jls")
...
savefig(plt, "iir_100x100_result.png")
println("Saved: iir_100x100_result.png")
